...
<details>
<summary>useEffect와 useLayoutEffect의 차이는 무엇인가요?</summary>

두 훅 모두 리액트의 Commit 단계 근처 실행되지만, 브라우저가 화면을 그리는 Paint 시점을 기준으로 차이가 발생합니다.

1. useLayoutEffect (동기적):
- 시점: 리액트가 DOM을 변경한 직후, 브라우저가 화면을 그리기(Paint) 직전에 실행됩니다.
- 특징: 이 훅 내부에서 DOM 수치를 측정하거나 변경하면 브라우저 Paint 전에 반영되므로, 화면 깜빡임(flicker)이 없습니다. 하지만 작업이 길어지면 화면이 늦게 뜨는 성능 저하가 생깁니다.

2. useEffect (비동기적):
- 시점: 브라우저가 Paint를 완료하여 사용자가 화면을 본 후에 실행됩니다.
- 특징: 브라우저 렌더링을 방해하지 않으므로 성능상 이점이 있습니다.
...
아래의 에러는 Error Boundary로 잡을 수 없습니다.
