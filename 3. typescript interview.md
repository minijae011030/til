...
TypeScript를 사용하면 컴파일 타임에 타입 오류를 잡을 수 있어 안정성이 높아지고, 코드 자동 완성, 리팩토링, 가독성 측면에서도 개발 생산성이 크게 향상됩니다. 특히 규모가 커질수록 유지보수에 유리합니다.
...
컴파일 타임 에러는 코드를 실행하기 전에 발생하는 오류로, TypeScript의 타입 오류가 이에 해당합니다. 반면 런타임 에러는 코드 실행 중에 발생하며, JavaScript에서 주로 발생합니다.
...
<details>
<summary>컴파일과 트랜스파일의 차이점을 아시나요?</summary>

컴파일은 한 언어의 소스 코드를 다른 언어(저수준의 언어)로 바꾸는 광범위한 의미입니다. (C++ -> 기계어)
...
둘 다 객체 타입을 정의할 수 있지만, interface는 확장과 선언 병합이 가능하고, type은 유니온, 인터섹션 등 더 다양한 타입 표현이 가능합니다.

객체 구조를 정의하고 확장이 필요한 경우에는 interface가 적합하고, 유니온 타입이나 복잡한 타입 조합이 필요한 경우에는 type이 더 적합합니다.
...
enum은 컴파일 후에도 JavaScript 코드로 남지만, union literal 타입은 타입 수준에서만 존재하고 런타임 코드가 생성되지 않습니다.
...
Pick은 기존 타입에서 필요한 속성만 선택해 새로운 타입을 만들 때 사용합니다. 반면 Omit은 기존 타입에서 특정 속성만 제외하고 나머지를 모두 사용하는 방식입니다.
...
public은 클래스 외부를 포함해 어디에서든 접근할 수 있는 접근 제어자로, 기본값이기도 합니다. private는 해당 클래스 내부에서만 접근이 가능하며, 내부 구현을 숨기는 데 사용됩니다. protected는 클래스 내부에서는 물론, 해당 클래스를 상속받은 자식 클래스에서도 접근할 수 있어 상속 구조에서 공통 로직을 공유할 때 주로 사용됩니다.
...
extends는 기존 타입이나 클래스의 기능/구조를 물려받아 확장하는 개념이고, implements는 클래스가 특정 인터페이스의 형태를 반드시 만족하도록 강제하는 개념입니다.
...
상속은 이미 존재하는 기능을 재사용하면서 일부를 변경하거나 추가해 확장하는 목적이 크고, 구현은 이 클래스는 이런 형태의 API를 반드시 제공한다라는 계약을 지키게 해서 설계를 일관되게 만드는 목적이 큽니다.
