...
클라이언트 상태에 accessToken을 들고 있게 되면 토큰이 항상 브라우저 환경에서 노출되기 때문에 XSS 공격에 취약해지고,
SSR 환경에서는 클라이언트 상태를 접근할 수 없어 인증 일관성이 깨질 수 있습니다.

그래서 저는 accessToken을 클라이언트 상태에 저장하지 않고, HttpOnly 쿠키만 서버에서 직접 읽어서 인증을 판단하는 구조로 선택했습니다.
이 방식은 JS로 쿠키를 읽을 수 없어 보안이 높고, 서버 컴포넌트나 라우트 핸들러가 요청 시점에서 바로 인증 여부를 판별할 수 있기 때문에 서버와 클라이언트가 동일한 인증 기준을 유지할 수 있다는 장점이 있습니다.
