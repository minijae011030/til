...
2. 불필요한 리렌더링 방지: **전역상태**를 효율적으로 관리합니다.
...
Zustand는 **Flux 패턴**을 따르는 가볍고 직관적인 상태 관리 라이브러리입니다. 기존 Redux의 복잡한 구조(보일러 플레이트)를 걷어내고 하나의 스토어에 상태와 액션을 함께 정의하는 단순한 방식을 채택하고 있습니다.

특히 Context API와 달리 전역적인 **Provider 지옥**을 만들지 않으며, 컴포넌트가 필요한 상태만 **선택적으로 구독**할 수 있게 하여 리렌더링 성능을 최적화하기에 매우 용이하다는 장점이 있습니다.
...
Flux 패턴은 데이터가 **Action -> Dispatcher -> Store -> View**라는 **단방향 흐름**으로만 움직이게 하는 아키텍처입니다.

![flux](https://github.com/minijae011030/til/blob/main/image/flux.png?raw=true)
...
✅ Action -> Dispatcher -> Store -> View, 단방향 데이터 흐름, 예측 가능성

</details>

<details>
<summary>Flux 패턴의 흐름에 대해서 설명해주실 수 있나요?</summary>

1. Action: 무엇이 일어날지 정의하는 '객체'입니다. (예: "아이템 추가", "이름 변경")
2. Dispatcher: 모든 **액션**을 받아서 **Store로 전달**하는 **중앙 허브**입니다.
3. Store: 애플리케이션의 상태와 상태를 변경하는 **로직**이 담긴 곳입니다.
4. View: Store에서 데이터를 가져와 화면에 보여주고, 사용자 입력이 들어오면 다시 Action을 발생시킵니다.

✅ 무엇이 일어날지 정의, 액션, 스토어 전달, 중앙 허브, 상태 변경 로직, 화면, 사용자 입력
...
<details>
<summary>Zustand 스토어에는 어떤 종류의 값을 저장하나요?</summary>

Zustand에는 전역적 의미를 가지는 상태만 넣습니다.

- 사용자 정보(user, role 등)
- 인증 토큰(accessToken)
- UI 상태(모달 open 여부, 사이드바 상태)

반면 페이지 내부에서만 사용하는 로컬 상태는 React의 useState로 관리해
스토어가 불필요하게 비대해지는 것을 방지합니다.

</details>

<details>
<summary>zustand persist는 어떤 역할을 하나요?</summary>

persist 미들웨어는 Zustand 스토어의 상태를 로컬 스토리지나 세션 스토리지에 자동으로 저장해주는 기능입니다.
앱을 새로고침해도 상태가 유지되기 때문에 로그인 정보, 다크모드 설정, 최근 검색어처럼 지속성이 필요한 상태를 관리할 때 유용합니다.

또한 partialize 옵션을 사용하면 저장하고 싶은 상태만 선택적으로 영구 저장할 수 있어 스토리지 오염을 방지할 수 있습니다.

</details>

<details>
<summary>Zustand의 Selector의 역할에 대해 설명해 주세요.</summary>
...
<details>
<summary>스토어의 상태를 업데이트할 때 불변성을 유지해야 하는 이유는 무엇인가요?</summary>

Zustand는 상태 변경 시 **얕은 비교(shallow compare)** 를 통해 변경 여부를 판단합니다.
그렇기 때문에 상태를 직접 변이하면 **이전 상태와 참조값이 동일해져 리렌더링이 트리거되지 않을** 수 있습니다.

불변성을 유지하면 **새로운 객체 참조**를 만들기 때문에 변경 감지가 확실하게 이루어지고, 상태 관리가 예측 가능해집니다.

✅ 얕은 비교, 직접 변경, 참조값, 리렌더링 트리거, 새로운 객체 참조, 예측 가능
...
- 서버 상태는 서버에서 관리되며 **네트워크 요청**을 통해 가져와야 하는 데이터로, 변경 시 **동기화와 캐싱 문제**가 발생합니다.
- 반면 클라이언트 상태는 UI 상태나 사용자 입력처럼 **애플리케이션 내부**에서 관리되며 즉시 변경되고 예측이 가능합니다.

✅ 서버 관리, 네트워크 요청, 동기화, 캐싱 문제, UI 상태, 사용자 입력, 애플리케이션 내부, 즉시 변경
...
React Query는 **서버 상태**를 효율적으로 관리하기 위한 라이브러리입니다.
기존의 useEffect + fetch 패턴에서 발생하던 **중복 요청, 로딩·에러 상태 관리, 캐싱 처리**의 복잡함을 줄이기 위해 등장했으며, 서버 데이터의 **캐싱과 동기화를 자동**으로 처리해줍니다.
...
React Query는 서버 데이터를 요청하는 라이브러리가 아니라, **서버 상태(server state)를 캐싱하고 동기화하며 생명주기**를 관리하는 상태 관리 라이브러리이기 때문입니다.
...
staleTime은 **데이터가 신선(fresh)하다고 간주되는 기간**입니다.
staleTime 동안에는 **재요청(Refetch)** 을 하지 않습니다.
불필요한 네트워크 요청 감소의 장점이 있습니다. (중복 요청 관리)

gcTime은 **쿼리가 더 이상 사용되지 않을 때(=inactive) 캐시가 메모리에서 제거되기까지의 시간**입니다. 컴포넌트가 언마운트되어 해당 데이터를 사용하는 곳이 아무도 없을 때부터 카운트 됩니다. (메모리 관리)
...
시간을 정하는 전략으로는 데이터의 **변경 빈도**를 최우선으로 고려합니다.
예를 들어, 거의 변하지 않는 '공지사항' 같은 데이터는 staleTime을 길게 설정합니다.
하지만 실시간성이 중요한 '주식 시세' 등은 staleTime을 0으로 설정해 항상 최신 데이터를 보장하도록 합니다.

또한, 사용자가 페이지를 나갔다 돌아왔을 때 로딩 스피너 대신 이전 데이터를 즉시 보여주기 위해
gcTime을 항상 staleTime보다 넉넉하게 설정하여 캐시를 최대한 활용하는 방향으로 UI를 변경할 수도 있습니다.
...
invalidateQueries는 특정 쿼리 캐시를 **오래된 데이터(stale) 상태**로 표시하여 다음 렌더링 시 자동으로 **refetch**를 트리거합니다.
...
- Query의 상태:
  ...
  - React Query가 **네트워크 요청**을 어떤 단계에서 처리 중인지 나타내는 상태입니다.
  - 컴포넌트 렌더링 과정에서 확인하는 값으로, 요청 중이면 loading, 실패하면 error, 응답이 정상적으로 도착하면 success 상태가 됩니다.

- 데이터의 상태:
  - 요청으로 받아온 데이터가 **캐시 안에서 어떤 생명주기**를 가지고 있는지 나타내는 **내부적 상태**입니다.
  - fetching 상태 (요청 발생) -> staleTime 동안 fresh (응답이 도착한 직후)에는 -> stale (staleTime 끝): 데이터 재요청 대상 -> inactive (컴포넌트에서 더 이상 해당 데이터를 사용하지 않음) -> gcTime 이후 캐시에서 삭제
...
1. 탐색 및 저장: 쿼리 키가 고유해야 전역적인 캐시 저장소에서 데이터를 정확히 찾아오거나 저장할 수 있습니다.
2. 데이터 리프레시: 키의 변화를 감지해 자동으로 데이터를 **리프레시**할 수 있습니다. (useEffect의 의존성 배열과 유사한 역할)
3. 무효화: 특정 데이터를 **무효화(Invalidate)할 때** 정확한 대상을 타겟팅하기 위해 고유한 키 관리는 필수적입니다

✅ 캐시 저장소의 고유 식별자, 탐색, 저장, 리프레시, 무효화
...
**Query Key Factory 패턴**을 사용했습니다.

1. 문자열 **하드코딩**을 방지해 오타나 키 중복으로 인한 **캐싱 버그**를 원천 차단할 수 있습니다.
2. 특정 데이터를 수정했을 때 해당 도메인의 관련 쿼리들만 **무효화(Invalidation)** 할 수 있어 효율적인 데이터 동기화가 가능했습니다

✅ 쿼리 키 팩토리, 하드코딩, 캐싱 버그, 도메인, 무효화
...
리액트 쿼리의 직렬화 과정은 JSON 기반이기 때문에 함수나 심볼(Symbol)처럼 직렬화할 수 없는 값은 무시되거나 에러가 발생할 수 있습니다.

따라서 쿼리 키에는 항상 **순수하게 직렬화 가능한 데이터(직합체, 원시값, 일반 객체)** 만 포함시키는 것이 안전합니다.
...
React Hook Form은 input 값을 React의 state로 관리하지 않고, **브라우저의 DOM이 가진 기본 값 저장 방식을 그대로 활용**합니다. 이 때문에 입력이 발생할 때마다 컴포넌트 전체가 리렌더링되지 않고, 오직 필요한 검증 단계에서만 React가 개입합니다.
...
React Hook Form은 폼 상태와 이벤트 처리를 담당하고, **Zod는 입력값의 유효성을 스키마 기반으로 검증**합니다.
...
<details>
<summary>React Hook Form의 watch를 사용하면 성능이 떨어지는 이유는 무엇인가요?</summary>

watch는 특정 필드의 변화를 실시간으로 추적하기 때문에, 해당 필드가 변경될 때마다 관련된 구독 컴포넌트들이 모두 리렌더링됩니다.
특히 여러 필드를 watch하면 Hook Form의 “리렌더 최소화 전략”이 무력화되어 성능이 크게 떨어질 수 있습니다.
그래서 watch는 꼭 필요한 부분에서만 제한적으로 사용하고 useWatch를 사용하는 것이 좋습니다.

</details>

<details>
<summary>React Hook Form의 register는 내부적으로 어떤 역할을 하나요?</summary>

register는 input 요소를 RHF 내부의 비제어 상태 관리 시스템에 연결해 ref를 등록하고,
값 변화·유효성 검증·dirty 체크 등을 추적할 수 있게 해줍니다.

이 과정에서 input의 value를 React state로 올리지 않기 때문에 리렌더링 없이도 값이 관리됩니다.

</details>

<details>
<summary>React Hook Form이 왜 controlled input(useState 기반)보다 효율적인지 아시나요?</summary>

useState 기반의 controlled input은 입력할 때마다 컴포넌트가 리렌더링되지만,
RHF는 비제어 방식으로 ref를 통해 값을 추적하기 때문에 입력해도 컴포넌트가 리렌더되지 않습니다.

또한 RHF는 필요한 순간에만 최소한의 렌더만 일으키도록 설계돼 있어 폼이 커질수록 성능 차이가 크게 납니다.
즉, DOM이 값을 관리하고 React는 변화에 따라 필요한 시점만 업데이트하기 때문에 더 가볍습니다.

</details>

<details>
<summary>React Hook Form의 controller는 언제 사용해야 하나요?</summary>

controller는 RHF가 비제어 방식으로 직접 관리할 수 없는 컴포넌트를 사용할 때 필요합니다.
UI 라이브러리 컴포넌트는 내부에서 value/onChange를 갖고 있어 controlled input처럼 동작하기 때문에 RHF가 중간에서 이를 감싸 관리하도록 도와줍니다.

즉, register로 연결할 수 없을 때 controller를 쓰면 됩니다.

</details>

<details>
<summary>React Hook Form의 onChange와 onBlur의 차이는 무엇인가요?</summary>

- onChange는 사용자가 입력할 때마다 실시간으로 검증을 수행해 빠른 피드백을 주는 반면,
- onBlur는 필드가 포커스를 벗어났을 때만 검증해 불필요한 검증 호출을 줄이고 사용자 경험을 부드럽게 합니다.

RHF는 mode 옵션을 통해 검증 시점을 유연하게 제어할 수 있고, 상황에 따라 성능·UX를 고려해 선택할 수 있습니다.
