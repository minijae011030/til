...
block 요소는 한 줄 전체를 차지하며 줄바꿈이 발생합니다. inline 요소는 콘텐츠 크기만큼만 차지하고 크기 지정이 불가능합니다. inline-block은 inline처럼 한 줄에 배치되지만 block처럼 크기 지정이 가능합니다.
...
display: none은 요소를 레이아웃에서 완전히 제거하지만, visibility: hidden은 공간은 유지한 채 화면에만 보이지 않게 합니다.
...
static은 기본 흐름에 따라 배치되고, relative는 자신의 원래 위치를 기준으로 이동합니다. absolute는 가장 가까운 위치 지정 부모를 기준으로 배치되며, fixed는 뷰포트 기준으로 고정됩니다. sticky는 스크롤 위치에 따라 relative와 fixed처럼 동작합니다.
...
!important가 가장 우선이며, 그 다음으로 인라인 스타일, ID 선택자, 클래스 선택자, 태그 선택자 순으로 적용됩니다. 동일한 우선순위일 경우 나중에 선언된 스타일이 적용됩니다.
