...
HTML은 **웹 페이지의 구조와 의미**를 정의하는 **마크업 언어**입니다. 텍스트, 이미지, 링크 같은 **콘텐츠가 어떤 역할**을 가지는지 브라우저와 검색엔진에 전달합니다.

✅ keyword: 웹페이지, 구조/의미, 마크업 언어, 컨텐츠, 역할, 브라우저/검색엔진
...
시맨틱 마크업은 태그가 가진 **의미**에 맞게 HTML 요소를 사용하는 것을 말합니다. 단순히 div로 구조를 나누는 것이 아니라 header, nav, section 같은 태그를 사용하면 문서의 구조와 의도가 명확해집니다. 이를 통해 **검색엔진**이 콘텐츠를 더 잘 이해할 수 있고, 스크린 리더를 사용하는 사용자에게도 **접근성**이 향상됩니다.
...
브라우저는 JavaScript가 DOM을 변경할 수 있기 때문에, HTML 파싱 도중 script 태그를 만나면 파싱을 멈추고 JavaScript를 먼저 실행합니다. 이 과정에서 화면 렌더링이 지연되는데, 이를 **렌더링 블로킹**이라고 합니다.
...
✅ keyword: HTML, 구조, 스타일/레이아웃, 구조-표현 분리, 유지보수성, 재사용성
...
- inline-block 요소는 inline 요소처럼 한 줄에 나란히 배치되지만, block 요소처럼 width와 height, margin, padding을 모두 지정할 수 있는 요소입니다. 대표적으로 img, button 같은 요소가 있으며, 가로로 배치하면서 크기 조절이 필요한 경우에 사용됩니다.
...
- display: none은 요소를 렌더 트리에서 완전히 제거하기 때문에 레이아웃에서 사라지고, 해당 요소와 관련된 **리플로우가 발생**합니다.
...
- 반면 visibility: hidden은 요소가 렌더 트리에 남아 있어 레이아웃 공간은 그대로 유지되고, 화면에만 보이지 않게 처리됩니다.
  이 경우 레이아웃 계산은 발생하지 않고 **리페인트만 발생**합니다.
...
- fixed는 항상 뷰포트를 기준으로 고정되는 반면,
- sticky는 특정 스크롤 위치에 도달하기 전까지는 일반 흐름을 따르다가 이후 고정됩니다.
...
- margin은 요소 바깥의 여백을 조절하고,
- padding은 요소 내부 콘텐츠와 테두리 사이의 여백을 조절합니다.
...
✅ keyword: 화면 크기/해상도, 레이아웃/스타일, 웹디자
...
미디어 쿼리 기반의 최소 너비 기준 **브레이크포인트**를 의미합니다.
...
<details>
<summary>유틸리티 클래스란 무엇인가요?</summary>

CSS에서 유틸리티 클래스는 딱 한 가지의 스타일 속성만 수행하도록 미리 만들어진 클래스를 의미합니다.
...
<details>
<summary>BEM과 CSS-in-JS와의 차이점을 아시나요?</summary>

BEM (Block Element Modifier): 전통적인 방법으로, 클래스 이름을 아주 상세하게 지어서 중복을 피하는 방식입니다. (.header\_\_menu-item--active)

CSS-in-JS: 대표적으로 Styled-Components가 있습니다. 스타일을 자바스크립트 변수처럼 선언합니다. props에 따라 배경색을 바꾸는 등의 동적 스타일링이 매우 강력합니다.
